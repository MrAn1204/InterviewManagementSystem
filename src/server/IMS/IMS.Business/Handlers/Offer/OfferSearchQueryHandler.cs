...
public class OfferSearchQueryHandler(IMapper mapper, IUnitOfWorks unitOfWork) : IRequestHandler<OfferSearchQuery, IEnumerable<OfferViewModel>>
{
    private readonly IMapper _mapper = mapper;
    private readonly IUnitOfWorks _unitOfWork = unitOfWork;

    public async Task<IEnumerable<OfferViewModel>> Handle(OfferSearchQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(request.candidateStatus))
        {
            query = query.Where(o => o.Candidate.Status.Contains(request.candidateStatus));
        }
        ...
        return _mapper.Map<IEnumerable<OfferViewModel>>(query);
    }
}
